-- Migration: Add Session Termination Function
-- Allows officers to manually terminate BLE sessions
-- Also handles automatic cleanup of orphaned sessions
...
-- 2. Create function to cleanup orphaned sessions (officer crashed/battery died)
...
BEGIN
    ...
    -- (ends_at is in the past but no termination marker)
    FOR session_record IN
        ...
        AND e.ends_at > NOW() - INTERVAL '24 hours' -- Only cleanup recent sessions
    LOOP
...
-- 3. Create function to get session status (for officer to check if session is still active)
...
-- 5. Create a scheduled job to cleanup orphaned sessions (runs every hour)
-- Note: This requires pg_cron extension to be enabled
-- Run this manually if pg_cron is available:
-- SELECT cron.schedule('cleanup-orphaned-ble-sessions', '0 * * * *', 'SELECT cleanup_orphaned_sessions()');
